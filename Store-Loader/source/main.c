...
bool checkForUpdate(char *cdnbuf)
{
	...
	if (if_exists("/user/app/RPIH00001/homebrew.elf") && if_exists("/user/app/RPIH00001/local.md5"))
	{
		logshit("[STORE_GL_Loader:%s:%i] ----- ELF exists ---\n", __FUNCTION__, __LINE__);
		logshit("[STORE_GL_Loader:%s:%i] ----- Comparing Hashs ---\n", __FUNCTION__, __LINE__);
		int comp = MD5_hash_compare("/user/app/RPIH00001/remote.md5", "/user/app/RPIH00001/local.md5");
...
copy_update:
	...
	unlink("/user/app/RPIH00001/homebrew.elf");
    unlink("/user/app/RPIH00001/local.md5");
        
     if(download_file(cdnbuf, "/user/app/RPIH00001/homebrew.elf") == 200)
     {
	     if (copyFile("/user/app/RPIH00001/remote.md5", "/user/app/RPIH00001/local.md5") != 0)
			 return false;
	     else{  
		   unlink("/user/app/RPIH00001/remote.md5");
...
void loader_rooted(){
    ...
    char buff[600];
		...
		mkdir("/user/app/RPIH00001/", 0777);
		mkdir("/user/app/RPIH00001/storedata/", 0777);
		mkdir("/user/app/RPIH00001/logs/", 0777);
		unlink("/user/app/RPIH00001/logs/loader.log");
 		...
 		if (if_exists("/mnt/usb0/settings.ini"))
		{
			...
			if (error) {
				...
				if (config.Copy_INI)
					copyFile("/mnt/usb0/settings.ini", "/user/app/RPIH00001/settings.ini");
			}
        ...
        else
        {
		    if (!if_exists("/user/app/RPIH00001/settings.ini"))
		    {
				...
				snprintf(&buff[0], sizeof buff, "[Settings]\nCDN=https://api.pkg-zone.com\nSecure_Boot=1\ntemppath=/user/app/RPIH00001/downloads\nStoreOnUSB=0\nShow_install_prog=1\nHomeMenu_Redirection=0\nDaemon_on_start=1\nLegacy=0\n");

				int fd = open("/user/app/RPIH00001/settings.ini", O_WRONLY | O_CREAT | O_TRUNC, 0777);
				if (fd >= 0)
				{
		    ...
		    else
		    {
				...
				int error = ini_parse("/user/app/RPIH00001/settings.ini", print_ini_info, &config);
				if (error) {
		...
		else if (ret == 0)
		{
			...
			if ((dl_ret = download_file(cdnbuf, "/user/app/RPIH00001/remote.md5")) == 200){
				...
				if (VerifyRSA)
				{
					...
					if (checkForUpdate(cdnbuf))
					{
						...
						if (config.SECURE_BOOT)
						{
							...
							if (update_version_by_hash("/user/app/RPIH00001/homebrew.elf"))
							...
							if (download_file(cdnbuf, "/user/app/RPIH00001/homebrew.elf.sig") == 200)
							{
								loadmsg(getLangSTR(RSA_LOAD));

								logshit("VerifyRSA resolved from PRX\n");

								if ((ret = VerifyRSA("/user/app/RPIH00001/homebrew.elf", "/mnt/sandbox/RPIH00001_000/app0/Media/rsa.pub")) != 0){
						...
						if (copyFile("/user/app/RPIH00001/homebrew.elf", "/data/self/Store.self") != 0) 
						    goto err;
						else
						   return;
					}
